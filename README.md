...
- 플레이어와 적 움직임에는 A*를 이용한 길찾기 알고리즘을 이용한다.
  ...
  ## 요구사항 [1년]
    ...
    * 아이템들은 떨어뜨리거나, 던질 수 있다.
    * 게임의 맵은 랜덤으로 정해지고, 각각 맵에는 전층으로 가는 계단과 다음층으로 가는 계단이 존재한다. 그리고 특정 층 이상 도달시, 다음 스테이지로 이동하낟.
    * 몹들은 플레이어의 시야 밖에서 생성된다.
    ...
    * 플레이어가 최종 스테이지를 클리어시 엔딩화면이 나오고, 기록된다. 그리고 습득 아이템을 전부 공개한다.
