...
* ### 그래픽
  ...
  * ### 엔딩화면
  ...
  ## [2주차] <a name = "num9">
    * <img width = "800" src = "게임프로젝트/2nd/2nd.gif"/>
  
  ### 작업명 : 오브젝트 랜덤 생성 밑 맵 전환
   ...
   ### 구체적 작업 내용 : 
    * 1주차에 못한 항목인 문 생성 및 계단 생성(80%)
      * 현재 벽을 따라 복도가 생성되는경우 문이 계속생성되는 경우를 해결하지 못함.
      * 새로 방없이 벽만 생성되는 곳이 생기는 현상이 발생.
    * 메인메뉴 UI 제작 (50%)
      * 제대로된 UI가 아닌 임시UI를 넣었음.
    * 메인메뉴 새로시작 버튼을 통해 게임화면으로 넘어가고, 계단 오브젝트를 통해 다른 게임화면으로 넘어가게 구현.
    * 맵이 생성될 떄 랜덤으로 생성된 방 안에 오브젝트를 생성, 이때 오브젝트들은 서로 다른 종류가 배치가되며, 복도와 벽에는 배치가 되지않음.
    * 좁은 맵이 생성될떄 오브젝트가 길을 막으면 안되므로 맵을 계산한 뒤 오브젝트를 배치한다.
    
  ## 3주차<a name = "num10">
    * <img width = "800" src = "게임프로젝트/3rd/3rd.gif"/>
 
  ### 작업명 : 맵제작 마무리 및 길찾기 알고리즘, 인터페이스 제작

      * 해당 요구사항 : (해당 요구사항 복사 붙여넣기)

      * 프로그래밍
           * 1) 바닥을 클릭하여 경로확인, 이동을 할 수 있다.

      * 그래픽
          *  1) 게임화면은 양손 드래그로 줌인, 줌아웃 할 수있다.
          *  2) 게임화면 에는 캐릭터이미지, HP바, 로그, 로그창, 인벤토리, 설정 버튼이 존재한다.

  ### 작업 내용 : (구체적 작업)

    * 1주, 2주차를 넘겨 해결하지 못한 맵만들기의 문제점 해결
        *  맵제작의 최소값, 최대값 설정 후 갯수제한, 비정상적인 형태를 띈 맵 생성하지않기.(100%)
        *  복도생성 후 양옆에 벽 생성하기.(100%)

        * "문"생성에 계속 오류가생겨서 문을 삭제함. 문을 제외하자 나머지가 정상적으로 작동

    * 오브젝트를 비교하여 타일단위의 길찾기 알고리즘 만들기. 
        * 현재 플레이어, 적 오브젝트가 없으므로 임시 오브젝트를 만들어 두 개 사이의 길을 잇는 알고리즘을 만들기.(100%)
        * 랜덤으로 생성되는 올라가는계단, 내려가는 계단 두개의 짧은 길 찾기를 실시함.

     * 카메라 이동 구현 (0%)
     * 게임 중 화면에 나오는 인터페이스 제작 (0%)
     * 인터페이스 내부의 기능은 작동하지 않지만, 새로 창을 띄우는 기능 구현. (0%)
  
  ## 6주차<a name = "num11">
    * <img width = "800" src = "게임프로젝트/6rd/2021-12-09 09-04-52.gif"/>
 
프로그래밍
1) 적을 클릭하여 공격할수 있다. 50%
   공격기능은 있으나 전투가 아직 구현되지 않아 아래 콘솔로만 표시가됨
2) 사정거리 밖일 경우 적에게 이동한다.
       * 플레이어, 적 오브젝트를 소환하여 플레이어와의 거리를 계산하여 적이 그 안에있을경우 이동이 아닌 공격을 진행, 그 후에 적도 동일하게 진행. 100%
3) 출구에 도착했을 시 다시 처음지점으로 돌아가며, 맵이 재생성된다. 50%
    재생성시 가끔 에러가 발생함.(그래서 현재 버튼으로만 구현됨)



요구사항에는 적혀있지 않는 내용 : 플레이어 주위를 제외하고는  화면이 보이지 않음
